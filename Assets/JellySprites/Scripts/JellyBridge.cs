			...
			if(jellySprite.m_Style == JellySprite.PhysicsStyle.Grid)				
			{
				...
				for(int x = 0; x < jellySprite.m_GridColumns; x++)	
				{
					...
					if(jellySprite.ReferencePoints[pointIndex].GameObject)
					{
						...
						if(jellySprite.ReferencePoints[pointIndex].GameObject.GetComponent<Rigidbody>())
						{
							jellySprite.ReferencePoints[pointIndex].GameObject.GetComponent<Rigidbody>().isKinematic = true;	
						}

						if(jellySprite.ReferencePoints[pointIndex].GameObject.GetComponent<Rigidbody2D>())	
						{
							jellySprite.ReferencePoints[pointIndex].GameObject.GetComponent<Rigidbody2D>().isKinematic = true;	
						}
					}
				}
			}
